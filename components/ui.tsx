...
export function PageHeader({ title, sub, actions }: { title:string; sub:string; actions?:ReactNode }) {
  return (
    <div style={{
      ...
      <div>
        <div style={{ fontFamily:"'Fredoka One',cursive", fontSize:20, color:'var(--text)', letterSpacing:'0.2px' }}>{title}</div>
...
export function Card({ children, style }: { children:ReactNode; style?: CSSProperties }) {
  return (
    <div style={{
      ...
      border:'1.5px solid rgba(240,98,146,0.14)',
...
export function CardHeader({ title, action }: { title:ReactNode; action?:ReactNode }) {
  return (
    <div style={{
      padding:'13px 16px',
      borderBottom:'1.5px solid rgba(240,98,146,0.10)',
      display:'flex', alignItems:'center', justifyContent:'space-between',
    }}>
...
export function Btn({ variant='primary', children, onClick, disabled, style, fullWidth, size='md' }:
  ...
  const pad = size==='sm'?'6px 13px':size==='lg'?'14px 22px':'9px 17px'
...
export function Badge({ children, bg, color }: { children:ReactNode; bg:string; color:string }) {
  return (
    <span style={{
      ...
      borderRadius:20, fontSize:11, fontWeight:800, background:bg, color,
    }}>
      {children}
    </span>
  )
...
export function StatCard({ icon, value, label, color }: { icon:ReactNode; value:string|number; label:string; color:string }) {
  return (
    <div style={{
      ...
      {/* topo colorido arredondado */}
      <div style={{ position:'absolute', top:0, left:0, right:0, height:4, background:color, borderRadius:'20px 20px 0 0' }} />
      <div style={{ marginBottom:8, color, marginTop:4 }}>{icon}</div>
      <div style={{ fontFamily:"'Fredoka One',cursive", fontSize:32, color:'var(--text)', lineHeight:1 }}>{value}</div>
      <div style={{ fontSize:11, color:'var(--sub)', marginTop:5, fontWeight:700 }}>{label}</div>
    </div>
  )
}
...
export function Avatar({ nome, fotoUrl, color, light, size=42, radius=12 }: {
  ...
  return (
    <div style={{
      ...
      background:fotoUrl?undefined:light,
      color:fotoUrl?undefined:color,
      fontSize:size*0.37,
...
export function Modal({ title, sub, children, footer, onClose }: {
  ...
  return (
    <div onClick={e=>{if(e.target===e.currentTarget)onClose()}} style={{
      position:'fixed', inset:0, background:'rgba(74,44,58,0.45)',
      ...
      <div className="animate-pop" style={{
        ...
        boxShadow:'0 24px 60px rgba(240,98,146,0.25)',
        border:'1.5px solid rgba(240,98,146,0.15)',
      }}>
        <div style={{ padding:'18px 22px', borderBottom:'1.5px solid rgba(240,98,146,0.12)', display:'flex', alignItems:'center', justifyContent:'space-between' }}>
          <div>
            <div style={{ fontFamily:"'Fredoka One',cursive", fontSize:19, color:'var(--text)' }}>{title}</div>
            {sub && <div style={{ fontSize:11, color:'var(--sub)', marginTop:2, fontWeight:600 }}>{sub}</div>}
          </div>
          <button onClick={onClose} style={{ width:32, height:32, borderRadius:10, background:'var(--pink-l)', border:'none', cursor:'pointer', display:'flex', alignItems:'center', justifyContent:'center', color:'var(--pink-d)', fontSize:14, fontWeight:900 }}>✕</button>
        </div>
        <div style={{ padding:'18px 22px', maxHeight:'60vh', overflowY:'auto', display:'flex', flexDirection:'column', gap:14 }}>{children}</div>
        {footer && <div style={{ padding:'14px 22px', borderTop:'1.5px solid rgba(240,98,146,0.12)', display:'flex', justifyContent:'flex-end', gap:8 }}>{footer}</div>}
      </div>
    </div>
  )
}
...
export function Field({ label, required, children }: { label:string; required?:boolean; children:ReactNode }) {
  return (
    <div>
      <label style={{ fontSize:11, fontWeight:900, color:'var(--text)', display:'block', marginBottom:5, letterSpacing:'0.2px' }}>
...
export function Empty({ msg }: { msg:string }) {
  return (
    <div style={{ padding:'36px 20px', textAlign:'center', color:'var(--sub)', fontSize:13, fontWeight:800 }}>
      {msg}
    </div>
  )
}
