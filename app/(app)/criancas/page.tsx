      ...
      <PageHeader title="Crianças" sub={`${criancas.length} cadastradas`}
        actions={
          <Btn onClick={()=>setModal(true)}>
            <PlusIcon style={{ width:14,height:14 }} /> Nova criança
          </Btn>
        }
      />
      <div style={{ flex:1, overflowY:'auto', padding:'16px 24px' }} className="animate-up">
        ...
        <div style={{ position:'relative', marginBottom:12 }}>
          ...
          <input value={busca} onChange={e=>setBusca(e.target.value)} placeholder="Buscar pelo nome..."
            style={{ width:'100%', padding:'11px 14px 11px 38px', border:'1.5px solid rgba(240,98,146,0.22)', borderRadius:14, fontFamily:"'Nunito',sans-serif", fontSize:13, fontWeight:600, color:'var(--text)', background:'rgba(255,255,255,0.85)', outline:'none', backdropFilter:'blur(6px)' }}
          />
        </div>
        ...
        <div style={{ display:'flex', gap:7, marginBottom:16, flexWrap:'wrap' }}>
          {[
            ...
            ...TURMAS_ORDER.map(id => ({
              id, label:`${TURMA_EMOJI[id]} ${TURMA_CONFIG[id].label} (${criancas.filter(c=>c.turma===id).length})`
            }))
          ].map(f => {
            ...
            return (
              <button key={f.id} onClick={()=>setFiltro(f.id)} style={{
                padding:'7px 15px', borderRadius:22, fontSize:11, fontWeight:800, cursor:'pointer',
        ...
        {lista.length===0
          ...
          : <div style={{ display:'flex', flexDirection:'column', gap:9 }}>
              {lista.map(c => {
                ...
                return (
                  <div key={c.id} onClick={()=>router.push(`/criancas/${c.id}`)} style={{
                    ...
                    <Avatar nome={c.nome} fotoUrl={c.foto_url} color={t.color} light={t.light} size={46} radius={14} />
                    <div style={{ flex:1, minWidth:0 }}>
                      <div style={{ fontSize:13, fontWeight:800, color:'var(--text)' }}>{c.nome}</div>
                      <div style={{ fontSize:11, color:'var(--sub)', fontWeight:600, marginTop:2 }}>
                        👪 {c.responsavel} &nbsp;·&nbsp; 📞 {c.telefone}
                      </div>
                    </div>
                    <div style={{ textAlign:'right', flexShrink:0, display:'flex', flexDirection:'column', alignItems:'flex-end', gap:6 }}>
                      <Badge bg={t.light} color={t.text}>{TURMA_EMOJI[c.turma]} {t.label}</Badge>
                      {f
