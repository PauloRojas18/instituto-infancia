...
export async function GET() {
  ...
  const { data: pt } = await supabase.from('presencas_turma').select('crianca_id, presente')
  const { data: pg } = await supabase.from('presencas_gerais').select('crianca_id, presente')

  const map: Record<number, { total:number; presentes:number }> = {}
  const add = (rows: { crianca_id:number; presente:boolean }[] | null) => {
    rows?.forEach(r => {
  ...
  criancas?.forEach(c => {
    ...
    const pct = f.total > 0 ? Math.round(f.presentes / f.total * 100) : null
    const t = TURMA_CONFIG[c.turma as keyof typeof TURMA_CONFIG]
    const row = ws.addRow({
      nome: c.nome, turma: t?.label ?? c.turma,
      responsavel: c.responsavel, telefone: c.telefone,
      presentes: f.presentes, total: f.total,
      freq: pct !== null ? `${pct}%` : '—',
      situacao: pct === null ? '—' : pct >= 75 ? '✓ Regular' : '⚠ Atenção',
    })
    if (pct !== null && pct < 75) {
      row.getCell('freq').font    = { bold:true, color:{ argb:'FFE24B4A' } }
      row.getCell('situacao').font = { bold:true, color:{ argb:'FFE24B4A' } }
    }
  })
  ...
  return new NextResponse(buf, {
    headers: {
      ...
      'Content-Disposition': `attachment; filename="frequencia_infancia.xlsx"`,
    },
  })
}